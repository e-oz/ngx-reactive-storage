...
describe('LocalStorage', () => {
		...
		Object.defineProperty(window, 'localStorage', {
			value: {
				...
				setItem: (key: string, value: string) => {
					...
					mockStorageEvent = new StorageEvent('storage', {
						key,
						newValue: value,
					});
	...
	describe('set', () => {
		it('sets the item in localStorage', async () => {
			...
			expect(localStorage.getItem(storage['prefixed'](key))).toBe(
				JSON.stringify(value)
			);
		});

		it('sets generic the item in localStorage', async () => {
			type My = {
				id: number;
				name: string;
			};
			...
			const storage = new RxLocalStorage();
			const key = 'testKey';
			const value = { id: 12, name: 'test' } satisfies My;
